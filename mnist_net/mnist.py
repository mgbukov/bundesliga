...
# define network's weights and biases
W = tf.Variable(tf.zeros([784,10]))
b = tf.Variable(tf.zeros([10]))
# initialise variables in session
sess.run(tf.initialize_all_variables())
# define log regression model
y = tf.matmul(x,W) + b

##### create a deep convolutional nets #####

# define new weights ad biases	
...
# transform data to 2D image: 28x28 pixels, with 1 color channel
...
sess.run(tf.initialize_all_variables())
for i in range(2000): # training steps
	batch = mnist.train.next_batch(50) # use minibatches
	if i%100 == 0:
